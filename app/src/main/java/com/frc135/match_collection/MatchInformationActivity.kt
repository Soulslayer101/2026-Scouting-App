...
open class MatchInformationActivity : CollectionActivity() {
    ...
    // Note: This function requires the calling activities to pass the EditText views as parameters
    // since they now use view binding instead of synthetic imports
    fun safetyCheck(
        view: View,
        currentScreen: String,
        etMatchNumber: EditText,
        etTeamOne: EditText? = null,
        etTeamTwo: EditText? = null,
        etTeamThree: EditText? = null
    ): Boolean {
        if (checkInputNotEmpty(etMatchNumber) and (allianceColor != Constants.AllianceColor.NONE)) {
            if (collectionMode == Constants.ModeSelection.OBJECTIVE) {
                if (currentScreen == "match_edit_activity_screen") {
                    if (etTeamTwo != null && checkInputNotEmpty(etTeamTwo)) {
                        if ((scoutId != Constants.NONE_VALUE)) {
                            //Ensures the inputted team contains at least 1 integer
                            if (stringContainsNumber(etTeamTwo.text.toString())) {
                                return true
                            } else {
                                createErrorMessage(
                                    message = getString(
                ...
                return if ((scoutId != Constants.NONE_VALUE)) {
                    //Ensures the inputted team contains at least 1 integer
                    if (etTeamOne != null && stringContainsNumber(etTeamOne.text.toString())) {
                        true
                    } else {
                        createErrorMessage(
                            message = getString(
                ...
                if (etTeamOne != null && etTeamTwo != null && etTeamThree != null &&
                    checkInputNotEmpty(etTeamOne, etTeamTwo, etTeamThree)) {
                    // Ensure that inputted team numbers are different.
                    return if ((etTeamOne.text.toString() != etTeamTwo.text.toString()) and
                        (etTeamOne.text.toString() != etTeamThree.text.toString()) and
                        (etTeamTwo.text.toString() != etTeamThree.text.toString())) {
                        // Ensures the inputted team numbers each contain at least 1 integer
                        if ((!stringContainsNumber(etTeamOne.text.toString()) ||
                                    !stringContainsNumber(etTeamTwo.text.toString()) ||
                                    !stringContainsNumber(etTeamThree.text.toString()))) {
                            createErrorMessage(
                                message = getString(
                                    R.string.team_name_missing_integer
                                ), view = view
